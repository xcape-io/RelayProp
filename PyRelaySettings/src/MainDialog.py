...
class MainDialog(AppletDialog):
    ...
    def __init__(self, title, icon, prop_settings, logger):
        ...
        if 'json' in self._propSettings['prop']:
            self._localFile = os.path.basename(self._propSettings['prop']['json'])
        ...
        if self._propSettings['prop']['board'] == 'mega':
            if 'mega_bridge' in self._propSettings['prop'] and self._propSettings['prop']['mega_bridge'] == '1':
                ...
                if not self._localFile:
                    self._localFileself._localFile = LOCAL_ARDUINO_MEGA2560_BRIDGE_JSON
            else:
        ...
        self._propSettings['prop']['json'] = os.path.abspath(self._localFile)
    ...
    def _buildUi(self):
        ...
        file_layout.addWidget(QLabel(self.tr("Local file:")))
        file_layout.addWidget(self._localFileDisplay)
        file_layout.addStretch()
        file_layout.addWidget(clear_button)
        file_layout.addWidget(print_button)
        main_layout.addLayout(file_layout)

        upload_button = QPushButton(" {} ".format(self.tr(" Upload settings ")))
    ...
    def upload(self):
        ...
        self.publishRetainedMessage.emit(self._propSettings['prop']['prop_settings'], settings)
    ...
    def uploadForMega(self):
        ...
        for key, pin in self._boardMegaPins:  # D0 and D1 must be cleared if set from previous config with no bridge
            pin_topic = self._propSettings['prop']['prop_settings'] + '/' + pin
