/*
  Name:    NucleoRelayProp.cpp
  Author:  Faure NucleoRelayProp <dev at faure dot systems>
  Editor:  https://github.com/fauresystems
  License: MIT License (c) Faure NucleoRelayProp <dev at faure dot systems>
...
NucleoRelayProp::NucleoRelayProp(const char* client_id,
                                 const char* in_box,
                                 const char* out_box,
...
void NucleoRelayProp::begin(void(*on_message)(String))
{
  if (on_message) onInboxMessageReceived = on_message;
}

void NucleoRelayProp::loop()
{
...
void NucleoRelayProp::addPin(String message)
{
...
void NucleoRelayProp::command(String a)
{
...
int NucleoRelayProp::pinCount()
{
  return _pinDataList.Count();
}

void NucleoRelayProp::removeAllPins()
{
  ...
  {
    delete list_of_references[k]->propData;
    delete list_of_references[k];
  }
}

void NucleoRelayProp::removePin(int pin)
{
...
void NucleoRelayProp::removePin(String action)
{
...
void NucleoRelayProp::setBrokerIpAddress(IPAddress ip, uint16_t port)
{
  _brokerIpAddress = ip;
